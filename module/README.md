...
原因是 `gg module copy` 按类型信息闭包复制 helper 文件（详见 `internal/ggmodule` 的包文档）：被复制文件引用了「声明 service struct 但不被任何 DSL action 复制」的文件时，copy 会在 preflight 直接报错，共享代码必须挪进独立 helper 文件才能通过。
