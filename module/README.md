...
开发可复制模块时，多个 service action 共享的函数、类型、常量不要放在某一个具体 action 的 service 文件里，例如 `verification_request.go`、`password_reset_request.go`、`change_confirm.go`。

这类代码应放到独立 helper 文件中，例如：

- `common.go`
- `<domain>_flow.go`
- `<domain>_helper.go`
- `<domain>_cancellation.go`
...
- action service 文件只放该 action 自己的 service struct、入口方法和强绑定该 action 的私有逻辑。
