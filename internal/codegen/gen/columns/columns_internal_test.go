...
func TestModuleRequiresGst(t *testing.T) {
	tests := []struct {
		name  string
		goMod string // empty when the project has no go.mod
		want  bool
	}{
		{
			name:  "requires the framework",
			goMod: "module example.com/app\n\ngo 1.27\n\nrequire github.com/hydroan/gst v1.0.0\n",
			want:  true,
		},
		{
			name:  "requires the framework next to a directive gg does not know",
			goMod: "module example.com/app\n\ngo 1.27\n\nnewdirective value\n\nrequire github.com/hydroan/gst v1.0.0\n",
			want:  true,
		},
		{
			name:  "names the framework only in a comment",
			goMod: "// Started from github.com/hydroan/gst.\nmodule example.com/app\n\ngo 1.27\n",
		},
		{
			name:  "declares a module path extending the framework path",
			goMod: "module github.com/hydroan/gst-demo\n\ngo 1.27\n",
		},
		{
			name:  "requires a module whose path extends the framework path",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Chdir(t.TempDir())
			if tt.goMod != "" {
				require.NoError(t, os.WriteFile("go.mod", []byte(tt.goMod), 0o600))
			}

			got, err := moduleRequiresGst()
			require.NoError(t, err)
