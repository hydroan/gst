...
func TestBuildCopyPlanCollectsStaleTargetModelFiles(t *testing.T) {
	projectDir := newModuleCopyPlanProject(t)
	writeCopyTestModuleSource(t, projectDir, nil)
	...
	staleTarget := filepath.Join(targetModelDir, "design.go")
	if writeErr := os.WriteFile(staleTarget, []byte("package copytest\n"), 0o600); writeErr != nil {
		t.Fatal(writeErr)
	}
	// Test files are project assets and generated files belong to their
	// generator (gg gen Cols files in particular), so neither may show up as a
	// stale target. The second generated header exercises the Go-wide
	// convention rather than gst's own header text.
	if writeErr := os.WriteFile(filepath.Join(targetModelDir, "design_test.go"), []byte("package copytest\n"), 0o600); writeErr != nil {
		t.Fatal(writeErr)
	}
	if writeErr := os.WriteFile(filepath.Join(targetModelDir, "design_cols.gen.go"), []byte("// Code generated by gst; DO NOT EDIT.\n\npackage copytest\n"), 0o600); writeErr != nil {
		t.Fatal(writeErr)
	}
	if writeErr := os.WriteFile(filepath.Join(targetModelDir, "mock_client.go"), []byte("// Code generated by MockGen. DO NOT EDIT.\n\npackage copytest\n"), 0o600); writeErr != nil {
		t.Fatal(writeErr)
	}
	...
	staleTargets := plan.StaleModelTargets()
	if len(staleTargets) != 1 {
		t.Fatalf("StaleModelTargets() = %v, want one stale target", staleTargets)
	}
	want := filepath.Join("model", "copytest", "design.go")
	if staleTargets[0] != want {
		t.Fatalf("StaleModelTargets()[0] = %q, want %q", staleTargets[0], want)
	}
}

func TestBuildCopyPlanCollectsStaleTargetServiceFiles(t *testing.T) {
	projectDir := newModuleCopyPlanProject(t)
	writeCopyTestModuleSource(t, projectDir, nil)
	...
	staleTarget := filepath.Join(targetServiceDir, "project_adapter.go")
	if writeErr := os.WriteFile(staleTarget, []byte("package copytest\n"), 0o600); writeErr != nil {
		t.Fatal(writeErr)
	}
	// Neither test files nor generated files may show up as stale targets;
	// see the model-side test for the exemption rationale.
	if writeErr := os.WriteFile(filepath.Join(targetServiceDir, "project_adapter_test.go"), []byte("package copytest\n"), 0o600); writeErr != nil {
		t.Fatal(writeErr)
	}
	if writeErr := os.WriteFile(filepath.Join(targetServiceDir, "project_mock.go"), []byte("// Code generated by MockGen. DO NOT EDIT.\n\npackage copytest\n"), 0o600); writeErr != nil {
		t.Fatal(writeErr)
	}
	...
	staleTargets := plan.StaleServiceTargets()
	if len(staleTargets) != 1 {
		t.Fatalf("StaleServiceTargets() = %v, want one stale target", staleTargets)
	}
	want := filepath.Join("service", "copytest", "project_adapter.go")
	if staleTargets[0] != want {
		t.Fatalf("StaleServiceTargets()[0] = %q, want %q", staleTargets[0], want)
	}
}
