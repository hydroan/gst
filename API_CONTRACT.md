...
| 游标分页 | `_cursor_value`、`_cursor_field`、`_cursor_next`；`_size` 控制批大小（缺省 20，上限 100）；时间列的 `_cursor_value` 同样只收 RFC 3339；游标与 offset 翻页互斥：仅嵌 `model.Cursor` 的资源传 `_page` 返回 400，游标生效时 `_page` 被忽略；使用游标时响应不返回 `total` | `model.Cursor` | `?_cursor_value=xxx&_cursor_next=true&_size=50` |
...
- 保存与他人的修改冲突时返回 HTTP `409`，`msg` 是可直接展示的提示：前端展示 `msg`
  并引导用户刷新页面后重新编辑即可，不要按 `msg` 的具体文案或 `code` 做逻辑分支。
