// The gst.yaml ignores as gg gen applies them: the model ignores, and the
// warnings both kinds of ignore rule raise. The route ignores are resolved
// with the routes themselves, in codegen.ResolveRoutes.
...
type modelIgnoreResult struct {
	...
	MultiSourceRules []codegen.MultiSourceRule
...
// codegen.ResolveRoutes so the live-action report sees the final action set.
func applyModelIgnores(allModels []*gen.ModelInfo, rules []ggconfig.ModelRule) modelIgnoreResult {
	...
	for _, m := range allModels {
		for i, rule := range rules {
			...
			matchedDirs[i][codegen.ModelRootDir(m.ModelFilePath)] = true
	...
	for i, rule := range rules {
		...
		if rule.From == "" && len(matchedDirs[i]) > 1 {
			...
			sort.Strings(dirs)
			result.MultiSourceRules = append(result.MultiSourceRules, codegen.MultiSourceRule{Raw: rule.Raw, Dirs: dirs})
		}
	}
	return result
}
