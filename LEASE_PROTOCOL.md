...
| `slot_ms` | bigint | 只有 cron 用：最近一次领走的调度时刻，只增不减 |
...
- 谁执行：到点的每个副本 · 启动补跑的副本
- 何时：调度时刻到了 / 启动时补最近一个没人领过的时刻（先单独读一次 `slot_ms`，分清是没人领过还是任务从没跑过）
...
| 消费者 | 一次完整过程 | 备注 |
| --- | --- | --- |
| cron | 「领取时刻」「跑这一轮」「续期 ×n」「登记跑完 / 被打断只放手」 | slot 只增不减，一个调度时刻全集群只领一次；上一轮还持有租约时下一个时刻被拒；丢租约后这一轮 5 秒不返回，进程立即退出 |
...
> 同步约定：本节两份清单与 gst 仓库 `README.md`「多副本部署 → 保证与边界」逐条对应，改这里的条目要同步改 README，README 改了也要同步回这里；每条下面的「实现」只写在这份协议文档里，不进 README。
