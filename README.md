...
同一个项目可以起多个副本而不改代码。需要"整个部署只做一次"的能力都建在主库的一张租约表
`gst_leases` 上，不依赖 Redis 或 etcd；项目用上其中任何一种能力后，这张表随 `gg migrate` 一起建。框架升级改了这张表
（比如加列）时同样先 `gg migrate` 再发布：没开 `auto_migrate` 的环境启动只检查表在不在，缺列要到领租约时才报错，定时任务一轮都跑不起来，选主和锁第一次用一个新名字时也领不到。启动期的两件事框架替你串行化：
多个副本对空库同时建表（`auto_migrate` 打开时），以及 `router.OnRoutesReady` 钩子里的播种，在整个部署里
一次只有一个副本在做（主库上的 advisory lock），所以播种照常写"先查后建"就行；钩子收到的是启动期上下文，
停机信号会取消它、钩子跑完它就结束，播种把它一路传给 dao 即可；排队的副本等多久由前面的副本
决定，每 30 秒记一条 WARN 说明自己在等，卡住的持锁副本靠编排器的启动探针重启来放开锁。这两把锁绑定
数据库会话：副本必须直连主库或走会话级连接池，经 ProxySQL / PgBouncer 这类按语句或事务复用连接的代理
锁会失效，持锁连接一断锁即释放。四种能力按"这件事长什么样"选：

| 能力 | 用在什么事上 | 语义 |
| --- | --- | --- |
| `cronjob.Register` | 到点做一次、做完就结束 | 表达式按 UTC 解释；每个调度时刻整个部署只领一次，其他副本跳过；fn 返回 nil、返回自己的错误或 panic 才算跑完；停机或丢租约时 fn 把 ctx 的结束（`ctx.Err()` 或 `context.Cause(ctx)`）原样返回，或者进程崩溃没能返回，这一轮算被打断，由先发现它的副本再跑一次（停机打断的约 15 秒内，崩溃的约 30 秒内；跑完了但记录跑完的语句失败的，和崩溃一样处理），只再跑一次，下一个时刻先开始了就放弃并记一条 WARN，所以 fn 必须幂等；启动时补跑最近一个没人领过的时刻；只该在本进程跑的用 `RegisterPerInstance` |
...
前三种建在租约上，共同的规则：租约每 2 秒续一次，续失败了按同样节奏重试，连续 10 秒没续上才算丢失，
数据库抖几秒不影响；fn 收到的 ctx 在停机或租约丢失时结束，fn 必须随之停下——丢租约后 5 秒还不返回，
进程立即以失败退出，不等停机排空、在途请求和其他组件，由编排器重启；在 ctx 上用 `database.Transaction`
开的事务会先核对租约，不是自己的就一条语句都不跑；普通的 Create / Update / Delete 不逐条核对，只靠 ctx
结束停下，和 Kubernetes 的 leader election 一样——它也不做逐写核对，靠续期截止早于租约到期加丢主即退出。
名字加上 `cron:`、`leader:`、`lock:` 前缀后不能超过 191 字节，超长在启动期报错。ClickHouse 当主库时
这三种能力启动即报错。本地用 SQLite 开发时框架只开一条连接，fn 里单个事务请控制在 8 秒内，事务一个接一个、
中间不停顿时每个控制在 5 秒内，更长的可能、超过 10 秒的必定挡住续期而按租约丢失处理。
