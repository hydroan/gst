...
（比如加列）时同样先 `gg migrate` 再发布：没开 `auto_migrate` 的环境启动只检查表在不在，缺列要到领租约时才报错，定时任务一轮都跑不起来。启动期的两件事框架替你串行化：
...
| 能力 | 用在什么事上 | 语义 |
| --- | --- | --- |
| `cronjob.Register` | 到点做一次、做完就结束 | 表达式按 UTC 解释；每个调度时刻整个部署只跑一轮，其他副本跳过；fn 返回 nil、返回自己的错误或 panic 才算跑完；停机或丢租约时 fn 把 ctx 的结束原样返回，或者进程崩溃没能返回，这一轮算被打断，由先发现它的副本再跑一次（停机打断的约 15 秒内，崩溃的约 30 秒内），只再跑一次，下一个时刻先开始了就放弃并记一条 WARN，所以 fn 必须幂等；启动时补跑最近一个没人领过的时刻；只该在本进程跑的用 `RegisterPerInstance` |
...
这三种能力启动即报错。本地用 SQLite 开发时框架只开一条连接，fn 里单个事务请控制在 8 秒内，更长的
可能、超过 10 秒的必定挡住续期而按租约丢失处理。
