...
进程会以失败退出，由编排器重启；在 ctx 上用 `database.Transaction` 开的事务会先核对租约，不是自己的
就一条语句都不跑；普通的 Create / Update / Delete 不逐条核对，只靠 ctx 结束停下，和 Kubernetes 的
leader election 一样——它也不做逐写核对，靠续期截止早于租约到期加丢主即退出。
名字加上 `cron:`、`leader:`、`lock:` 前缀后不能超过 191 字节，超长在启动期报错。ClickHouse 当主库时
这三种能力启动即报错。本地用 SQLite 开发时框架只开一条连接，fn 里单个事务请控制在 5 秒内，否则
续期被挡住会按租约丢失处理。
