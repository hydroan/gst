...
`gst_leases` 上，不依赖 Redis 或 etcd；项目用上其中任何一种能力后，这张表随 `gg migrate` 一起建，表结构有变化时
和项目自己的表一样先 `gg migrate` 再发布：没开 `auto_migrate` 的环境启动只检查表在不在，缺列要到领租约时才报错，定时任务一轮都跑不起来，选主和锁第一次用一个新名字时也领不到。启动期的两件事框架替你串行化：
