...
func TestDatabaseWithDB(t *testing.T) {
	...
	require.Empty(t, users)
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db3).List(&users))
	require.Empty(t, users)

	// Create resources in db2
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).Create(ul...))
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).List(&users))
	require.Len(t, users, 3)
	...
	user := new(TestUser)
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).Get(user, u1.ID))
	...
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).Update(user))
	updatedUser := new(TestUser)
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).Get(updatedUser, u1.ID))
	require.Equal(t, "user1_updated", updatedUser.Name)
	user.Name = "user1" // restore

	// Create resources in db3
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db3).Create(ul...))
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).List(&users))
	...
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).Delete(ul...))
	require.NoError(t, database.Database[*TestUser](context.Background()).List(&users))
	require.Empty(t, users)
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).List(&users))
	require.Empty(t, users)
	...
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db3).Delete(ul...))
	require.NoError(t, database.Database[*TestUser](context.Background()).List(&users))
	require.Empty(t, users)
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).List(&users))
	require.Empty(t, users)
...
func TestDatabaseWithTable(t *testing.T) {
	...
	require.Error(t, database.Database[*TestUser](context.Background()).WithDB(db2).WithTable("test_users").Create(u1))
	require.Error(t, database.Database[*TestUser](context.Background()).WithDB(db3).WithTable("test_users").Create(ul...))

	// Manually migrate the database.
	require.NoError(t, db2.AutoMigrate(TestUser{}))
	require.NoError(t, db3.AutoMigrate(TestUser{}))
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).WithTable("test_users").Create(ul...))
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db3).WithTable("test_users").Create(ul...))

	users := make([]*TestUser, 0)
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).WithTable("test_users").List(&users))
	require.Len(t, users, 3)
	// Verify data integrity
	var foundU1, foundU2, foundU3 bool
	for _, u := range users {
	...
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db3).WithTable("test_users").List(&users))
	require.Len(t, users, 3)

	// Get operation with custom table
	user := new(TestUser)
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).WithTable("test_users").Get(user, u1.ID))
	require.NotNil(t, user)
	require.Equal(t, u1.ID, user.ID)
	require.Equal(t, u1.Name, user.Name)
	...
	user.Name = "user1_updated"
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).WithTable("test_users").Update(user))
	updatedUser := new(TestUser)
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).WithTable("test_users").Get(updatedUser, u1.ID))
	require.Equal(t, "user1_updated", updatedUser.Name)

	// Delete operation with custom table
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).WithTable("test_users").Delete(u1))
	require.NoError(t, database.Database[*TestUser](context.Background()).WithDB(db2).WithTable("test_users").List(&users))
...
func TestDatabaseWithDebug(t *testing.T) {
	...
	t.Run("Create", func(t *testing.T) {
		defer cleanupTestData()
		require.NoError(t, database.Database[*TestUser](context.Background()).WithDebug().Create(ul...))
		users := make([]*TestUser, 0)
		require.NoError(t, database.Database[*TestUser](context.Background()).List(&users))
		require.Len(t, users, 3)
		// Verify data integrity
	...
	t.Run("List", func(t *testing.T) {
		...
		setupTestData(t)
		users := make([]*TestUser, 0)
		require.NoError(t, database.Database[*TestUser](context.Background()).WithDebug().List(&users))
		require.Len(t, users, 3)
	})
	...
	t.Run("Get", func(t *testing.T) {
		...
		setupTestData(t)
		user := new(TestUser)
		require.NoError(t, database.Database[*TestUser](context.Background()).WithDebug().Get(user, u1.ID))
		require.NotNil(t, user)
		require.Equal(t, u1.ID, user.ID)
		require.Equal(t, u1.Name, user.Name)
	...
	t.Run("Update", func(t *testing.T) {
		...
		user.Age = 25
		require.NoError(t, database.Database[*TestUser](context.Background()).WithDebug().Update(user))
		users := make([]*TestUser, 0)
		require.NoError(t, database.Database[*TestUser](context.Background()).List(&users))
		require.Len(t, users, 3)
		for _, u := range users {
	...
	t.Run("Delete", func(t *testing.T) {
		defer cleanupTestData()
		setupTestData(t)
		require.NoError(t, database.Database[*TestUser](context.Background()).WithDebug().Delete(ul...))
		users := make([]*TestUser, 0)
		require.NoError(t, database.Database[*TestUser](context.Background()).List(&users))
		require.Empty(t, users)
	})
	...
	t.Run("First", func(t *testing.T) {
		...
		require.NoError(t, database.Database[*TestUser](context.Background()).WithDebug().First(firstUser))
