...
func TestGetAnswersWhatThisReplicaHolds(t *testing.T) {
	...
	missing, err := cli.Get[model.CachedRsp](t.Context(), "/api/caches/never-written-key")
	require.NoError(t, err)
	require.False(t, missing.Found, "a key this replica never received reads as missing")
	require.Empty(t, missing.Value)

	_, err = cli.Post[model.CachedRsp](t.Context(), "/api/caches", model.CachedReq{Key: "read-key", Value: "read-value"})
	require.NoError(t, err)

	read, err := cli.Get[model.CachedRsp](t.Context(), "/api/caches/read-key")
	require.NoError(t, err)
