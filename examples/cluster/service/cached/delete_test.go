...
func TestDeleteRemovesTheEntry(t *testing.T) {
	...
	_, err = cli.Post[model.CachedRsp](t.Context(), "/api/caches", model.CachedReq{Key: "deleted-key", Value: "deleted-value"})
	require.NoError(t, err)

	removed, err := cli.Delete[model.CachedRsp](t.Context(), "/api/caches/deleted-key", nil)
	require.NoError(t, err)
	require.Equal(t, "deleted-key", removed.Key)

	read, err := cli.Get[model.CachedRsp](t.Context(), "/api/caches/deleted-key")
	require.NoError(t, err)
