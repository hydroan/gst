...
# MySQL and three replicas — and waits until every pod is ready. The image is
# built from the repository root, since the example's go.mod points at the
# framework sources beside it. A cluster that cannot see the local Docker
