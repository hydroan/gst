...
// no request body, matching the framework contract that List and Get actions
// declare no Payload.
func Get[RSP any](cli *Client, path string, opts ...RequestOption) (*RSP, error) {
	return callVerb[RSP](cli, http.MethodGet, path, nil, opts)
}

// Post sends a POST request and decodes the envelope data into RSP.
...
// Put sends a PUT request and decodes the envelope data into RSP.
...
// Patch sends a PATCH request and decodes the envelope data into RSP.
...
// Delete sends a DELETE request and decodes the envelope data into RSP. The
// payload covers the standard batch delete body; deleting by id passes nil.
