...
func TestVerbsDecodeEnvelopeData(t *testing.T) {
	...
	got, err := cli.Get[sampleRecordRsp](t.Context(), "/api/records/one")
	require.NoError(t, err)
	require.Equal(t, "GET-one", got.Name)

	got, err = cli.Post[sampleRecordRsp](t.Context(), "/api/records/two", map[string]string{"name": "two"})
	require.NoError(t, err)
	require.Equal(t, "POST-two", got.Name)

	got, err = cli.Put[sampleRecordRsp](t.Context(), "/api/records/three", nil)
	require.NoError(t, err)
	require.Equal(t, "PUT-three", got.Name)

	got, err = cli.Patch[sampleRecordRsp](t.Context(), "/api/records/four", nil)
	require.NoError(t, err)
	require.Equal(t, "PATCH-four", got.Name)

	got, err = cli.Delete[sampleRecordRsp](t.Context(), "/api/records/five", nil)
	require.NoError(t, err)
...
func TestVerbsDecodeEmptyDataToZeroValue(t *testing.T) {
	...
	got, err := cli.Post[struct{}](t.Context(), "/api/records", nil)
	require.NoError(t, err)
...
func TestListResultDecodesItemsAndTotal(t *testing.T) {
	...
	list, err := cli.Get[client.ListResult[sampleRecordRsp]](t.Context(), "/api/records")
	require.NoError(t, err)
...
func TestGetSendsNoBody(t *testing.T) {
	...
	got, err := cli.Get[sampleRecordRsp](t.Context(), "/api/records")
	require.NoError(t, err)
