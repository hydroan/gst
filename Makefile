...
# prints a line per package and none for a package without tests, testv prints
# what go test -v does, and every run ends with its totals and the output of
# the tests that failed. A machine without gotestsum gets the version go.mod
# pins installed on the first run.
...
TEST_OUTPUT := --format pkgname --format-hide-empty-pkg
TESTV_OUTPUT := --format standard-verbose
...
testv:
	...
	$(call run_tool,gotestsum,$(TESTV_OUTPUT) -- $(TEST_FLAGS) ./...)
	@echo "Running the per-dialect suites against postgres with verbose output..."
	$(call run_tool,gotestsum,$(TESTV_OUTPUT) -- $(TEST_FLAGS) -tags gsttest_postgres $(DIALECT_PACKAGES))
	@echo "Running the per-dialect suites against sqlite with verbose output..."
	$(call run_tool,gotestsum,$(TESTV_OUTPUT) -- $(TEST_FLAGS) -tags gsttest_sqlite $(DIALECT_PACKAGES))
	@echo "Running example project tests with verbose output..."
	$(call run_tool_in,gotestsum,examples/demo,$(TESTV_OUTPUT) -- $(TEST_FLAGS) ./...)
	$(call run_tool_in,gotestsum,examples/cluster,$(TESTV_OUTPUT) -- $(TEST_FLAGS) ./...)
