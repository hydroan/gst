...
# Every run carries the race detector. The framework runs the concurrency a
# project relies on -- renewal goroutines, campaign loops, scheduler loops,
# component and lifecycle startup -- and a data race there surfaces as a
# failure somewhere else entirely, which review does not catch.
...
test:
	...
	go -C examples/demo test -race ./...
	go -C examples/cluster test -race ./...
...
testv:
	...
	go -C examples/demo test -race -v ./...
	go -C examples/cluster test -race -v ./...
