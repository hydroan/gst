...
5. 批次边界不许顺手扩大，也不许沉默跳过。途中发现的计划外问题分两类：不碰框架代码逻辑的小问题（注释、文档、测试用例的缺漏、命名与位置，以及 `examples/` 下的一切）当批直接修，在汇报里列出；碰框架代码逻辑的（真 bug、坏味道、性能与安全隐患、可优化点）当场记下，在本批汇报里给出位置、现象、影响，不另编批次，我批准后在当前批次结束后接着处理，没答复前不要动它。两类都不许只自己知道不报。
