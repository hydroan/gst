...
4. 代码注释使用英文，注释必须符合 golang 规范、习惯。开发过程中发现现有注释有问题、过时或不符合代码逻辑的，即使与当前任务无关，也必须顺手修复。生成代码的函数（AST 拼装或字符串拼接），注释要写出它实际生成的代码，并由一条输入相同的测试用例逐字守住（用例注释标明对应哪个函数）；推导名字、路径、别名这类规则函数，注释给出具体的输入和输出。
