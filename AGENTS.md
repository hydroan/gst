...
- 改索引名必须"先 `gg migrate` 后发布"：开启 auto_migrate 的环境先发布，启动期会发现同定义的索引还挂在旧名下，直接报错退出并给出改名语句（启动期不 DROP 任何索引）；migrate 计划前的疑似改名提示应优先按指引执行改名语句（advisory 按方言给出：MySQL `RENAME INDEX`，PostgreSQL `ALTER INDEX ... RENAME TO`）。
