...
1. 先给总览再动手：每批列出目标、涉及文件、有无行为变更、验收命令、review 关注点、预估 diff 大小。我可能只批其中几批，不要默认全批通过。
2. 拆批的第一原则是「行为变不变」：重命名、文件搬运、注释和 TODO 归零行为批；纯重构（抽公共函数、收敛重复实现）单独一批，用「测试断言一字不改仍全绿」作为没改行为的证据；改变导出 API 语义、生成产物或框架契约的独占一批；防护性改动（上限、校验、错误口径）再单独一批。批次间有依赖就按依赖排序，把核心批的 diff 压到最小。
3. 每批自成闭环：改完 `make check` 通过、相关包测试全绿，仓库在任意一批结束时都是可用状态。
4. 一批做完停下汇报，等我明确说「继续」再做下一批；批准某一批不等于批准后续批次，方案因我的答复变了就重新呈现再等批准。
5. 批次边界不许顺手扩大，也不许沉默跳过：修 bug、重构、加能力途中发现的任何计划外问题（真 bug、坏味道、过时或错误的注释、缺失的测试、性能与安全隐患、可优化点），当场记下来，在本批汇报里给出位置、现象、影响，以及「单独成批 / 建议并入哪一批 / 建议不做」的判断，由我决定要不要进批次。我没答复前不要动它，也不要只自己知道不报。
6. 需要拍板的点（命名、口径、取舍）既不要自己定也不要卡住：列选项、给推荐方案和理由、说清影响面，然后问我。
