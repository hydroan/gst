...
func TraceID() gin.HandlerFunc {
	return func(c *gin.Context) {
		traceID := c.Request.Header.Get(consts.HEADER_TRACE_ID)
		pspanID := c.Request.Header.Get(consts.HEADER_SPAN_ID)
		spanID := util.SpanID()
		if len(traceID) == 0 {
			// If trace ID is empty, it means that it is the first request.
			traceID = spanID
		}
